...
class FlowGen(nn.Module):
  def __init__(self, input_dim=3, dim=64, n_res=2, activ='relu',
    ...
    self.conv_column = ConvColumn(27, dim, n_res, activ,
                                  norm_conv, pad_type, use_sn)

  def forward(self, parsing):
    images_out = self.conv_column(parsing)
    return images_out
...
class ConvColumn(nn.Module):
  def __init__(self, input_dim=3, dim=64, n_res=2, activ='lrelu',
    ...
    self.up_sample += [(nn.Sequential(
      nn.Upsample(scale_factor=2),
      Conv2dBlock(dim // 8, 9, 3, 1, 1, norm, 'none', pad_type, use_sn=use_sn)))]  # 128-256
...
class InpaintGenerator(BaseNetwork):
  def __init__(self, init_weights=True):  # 1046
    ...
    self.flow_param = {'input_dim': 3, 'dim': 64, 'n_res': 2, 'activ': 'relu',
                       'norm_conv': 'ln', 'norm_flow': 'none', 'pad_type': 'reflect', 'use_sn': False}
  ...
  def forward(self, pose, parsing):
    # encoder

    outputs = self.f_gen(torch.cat((pose, parsing), dim=1))
