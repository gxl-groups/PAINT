...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        parsing_im_b_20 = torch.zeros(4, 256, 256)
        parsing_im_b_20[0] += (parsing == 0).float()
        for i in range(1, 4):
            parsing_im_b_20[i] += (parsing == i + 4).float()
        ...
        pose_maps = torch.randn((self.point_num, self.height, self.width))
