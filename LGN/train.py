    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.display_freq == 0:   # display images on visdom and save images to a HTML file
                ...
                vis['real_b_encoded'] = generate_label(vis['real_b_encoded'], 256, 256)
                vis['fake_B_random'] = generate_label(vis['fake_B_random'], 256, 256)
                vis['fake_B_encoded'] = generate_label(vis['fake_B_encoded'], 256, 256)
                visualizer.display_current_results(vis, epoch, save_result)
